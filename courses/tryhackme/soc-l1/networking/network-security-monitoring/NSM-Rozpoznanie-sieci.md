...
![alt](/courses/tryhackme/soc-l1/networking/network-security-monitoring/Attachments/2-image.png)

**Odp. 80, 445, 3389**

# Mechanika Skanowania

## Ping Sweep

To jedna z najbardziej podstawowych technik skanowania sieci. **Ping sweep** jest zazwyczaj używany do identyfikowania hostów obecnych w sieci i sprawdzania, czy są aktywne. Taki skan polega na wysłaniu do hosta pakietu **ICMP (Internet Control Message Protocol)**. Jeśli host jest online, odpowie własnym pakietem ICMP. Obecnie jednak w wielu organizacjach ten typ ruchu jest blokowany przez mechanizmy bezpieczeństwa, co ułatwia obronę przed takim skanowaniem.

## Skanowanie TCP SYN

Połączenie TCP jest inicjowane przez **trójetapowy handshake**, który przebiega według sekwencji:

**SYN → SYN-ACK → ACK**

Skanery sieciowe mogą wykorzystywać ten mechanizm do identyfikowania aktywnych hostów i ich otwartych portów. Skaner wysyła żądanie **SYN** do odbiorcy. Jeśli otrzyma odpowiedź **SYN-ACK**, oznacza to, że host jest online, a port, do którego wysłano żądanie SYN, jest otwarty.

Jest to **cichy (stealthy) typ skanowania**, który często zlewa się z normalnym ruchem sieciowym i jest trudniejszy do wykrycia.

## Skanowanie UDP

Innym sposobem identyfikowania aktywnych hostów i otwartych portów jest wysyłanie pustego lub prawie pustego pakietu **UDP**. Jeśli port jest zamknięty, host odsyła komunikat **ICMP port unreachable**. Oznacza to, że port jest zamknięty, ale sam host jest aktywny.

W niektórych przypadkach skaner nie otrzymuje żadnej odpowiedzi aż do upływu ustalonego limitu czasu. Jeśli odpowiedź nie nadejdzie, skaner oznacza port jako **otwarty**, ale nie jest to jednoznaczny dowód, że port rzeczywiście jest otwarty. W rzadkich przypadkach skaner może otrzymać odpowiedź w postaci pakietu UDP, co stanowi potwierdzenie, że port jest otwarty.

Jak można z tego wywnioskować, **skanowanie UDP jest mało niezawodne i wolne**, ponieważ opiera się na oczekiwaniu na brak odpowiedzi aż do wystąpienia timeoutu.

## Wewnętrzne skanowania w organizacjach

Większość organizacji regularnie przeprowadza **wewnętrzne skanowania**, aby:

- sprawdzać podatności
- monitorować nieautoryzowane lub nieznane zasoby
- identyfikować możliwości zmniejszenia powierzchni ataku

Dla analityka SOC istotne jest, aby znać szczegóły takich skanów, w tym:

- adres IP, z którego są uruchamiane
- typ wykonywanego skanowania
- harmonogram skanowania

Najlepiej, aby takie legalne skanowania były **wyłączone z reguł detekcyjnych SOC**, co pozwala ograniczyć szum i liczbę fałszywych alarmów.

## Identyfikowanie typów skanowania

Sprawdźmy teraz, czy da się rozpoznać typy skanów na podstawie logów firewalla. Zrobimy to z użyciem **Kibany**. 

Przejdź do **menu hamburgera** w lewym górnym rogu i kliknij **Discover**.

![alt](/courses/tryhackme/soc-l1/networking/network-security-monitoring/Attachments/3-image.png)

W lewym górnym rogu, w polu **Data View**, wybierz **All logs**, a następnie ustaw opcję **Search entire time range**, aby wyświetlić logi z całego dostępnego zakresu czasu.

![alt](/courses/tryhackme/soc-l1/networking/network-security-monitoring/Attachments/4-image.png)

Gdy wszystkie logi będą już widoczne, można użyć znaku **„+”** widocznego przy każdym polu w lewej kolumnie, aby dodać je do wyników wyszukiwania jako osobną kolumnę.

![alt](/courses/tryhackme/soc-l1/networking/network-security-monitoring/Attachments/5-image.png)

Na końcu można użyć kontrolek dostępnych przy każdej wartości, aby:
- filtrować wyniki tylko dla tego pola
